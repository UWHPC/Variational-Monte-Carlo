...
EnergyTracker::EnergyTracker(real_t box_length, std::size_t num_particles)
: box_length_{box_length}
, ewald_alpha_{6.0_r / box_length}
, ewald_correction_{-6.0_r * static_cast<real_t>(num_particles)/(xpu::sqrt(std::numbers::pi_v<real_t>) * box_length)}
, ewald_background_{-std::numbers::pi_v<real_t>*static_cast<real_t>(num_particles)*static_cast<real_t>(num_particles)/(72.0_r * box_length)}
, V_recip_{}
, V_real_{}
, num_g_vectors_{}
, data_{0uz}
, reduction_scratch_{1uz}
{
  ...
  std::vector<real_t> g_x{};
  std::vector<real_t> g_y{};
  std::vector<real_t> g_z{};
  ...
  data_ = xpu::soa<real_t, idx(ArrayIndex::NUM_ARRAYS)>{
    this->num_g_vectors()
  };
...
void EnergyTracker::initialize_reciprocal_energy() noexcept {
  ...
  const real_t prefactor{
    1.0_r / (2.0_r * std::numbers::pi_v<real_t> * L * L * L)
  };
