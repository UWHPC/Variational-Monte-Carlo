...
std::pair<fp_t, fp_t> BlockingAnalysis::mean_and_standard_error() const {
  ...
  const fp_t variance{running_m2_ / scast<fp_t>(num_blocks_ - 1)};
  const fp_t standard_error{xpu::sqrt(variance / scast<fp_t>(num_blocks_))};
...
void BlockingAnalysis::add(fp_t local_energy) {
  ...
  if (in_block_ == block_size_) {
    const fp_t block_mean{block_sum_ / scast<fp_t>(block_size_)};

    ++num_blocks_;

    const fp_t delta{block_mean - running_mean_};
    running_mean_ += delta / scast<fp_t>(num_blocks_);
