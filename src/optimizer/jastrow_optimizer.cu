...
JastrowOptimizer::Result JastrowOptimizer::optimize(const Config& base_config, bool verbose) {
  const fp_t R_S{compute_rs(base_config)};
  ...
  const fp_t b_min{1.0_fp / R_S};
  const fp_t b_max{std::max(5.0_fp, b_min + 0.5_fp)};
  ...
  fp_t best_b{1.0_fp};
  fp_t best_energy{std::numeric_limits<fp_t>::max()};
  fp_t second_best_energy{std::numeric_limits<fp_t>::max()};
  ...
  const bool at_boundary{xpu::abs(best_b - b_min) < 1e-10_fp};
  if (at_boundary && results.size() > 2) {
    // Compute the energy spread of all non-boundary points
    fp_t interior_min{std::numeric_limits<fp_t>::max()};
    fp_t interior_max{std::numeric_limits<fp_t>::lowest()};
    for (const auto& r : results) {
      if (xpu::abs(r.b - b_min) < 1e-10_fp)
        continue;
      interior_min = std::min(interior_min, r.energy);
      interior_max = std::max(interior_max, r.energy);
    }
    const fp_t interior_spread{interior_max - interior_min};
    const fp_t boundary_gap{interior_min - best_energy};
  ...
  return Result{.optimal_b = best_b, .energy = best_energy, .standard_error = 0.0_fp};
}
...
JastrowOptimizer::EvalResult JastrowOptimizer::evaluate(
  ...
  cfg.step_size = base_config.box_length / 10.0_fp;
  ...
  return EvalResult{
    ...
    .standard_error = summary.standard_error.value_or(0.0_fp)
  };
}