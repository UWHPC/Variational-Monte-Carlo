...
#ifdef FP_64
constexpr fp_t MATRIX_TOLERANCE{1e-9_fp};
#else
constexpr fp_t MATRIX_TOLERANCE{1e-4_fp};
#endif

TEST_CASE("LU decomposition solves a non-singular system", "[matrix]") {
  constexpr std::size_t N{3};
  std::array<fp_t, N * N> A{
      2.0_fp, 1.0_fp, 1.0_fp, 4.0_fp, 3.0_fp, 3.0_fp, 8.0_fp, 7.0_fp, 9.0_fp,
  };
  std::array<fp_t, N> b{7.0_fp, 19.0_fp, 49.0_fp};
  ...
  REQUIRE(std::abs(x[0] - 1.0_fp) < MATRIX_TOLERANCE);
  REQUIRE(std::abs(x[1] - 2.0_fp) < MATRIX_TOLERANCE);
  REQUIRE(std::abs(x[2] - 3.0_fp) < MATRIX_TOLERANCE);
}

TEST_CASE("LU back-substitution stays finite on a singular matrix", "[matrix]") {
  constexpr std::size_t N{3};
  std::array<fp_t, N * N> A{
      1.0_fp, 2.0_fp, 3.0_fp, 2.0_fp, 4.0_fp, 6.0_fp, 1.0_fp, 1.0_fp, 1.0_fp,
  };
  std::array<fp_t, N> b{1.0_fp, 1.0_fp, 1.0_fp};
  std::array<int, N> pivot{};
  std::array<fp_t, N> x{};
