...
fp_t energy_at_b(std::size_t N, fp_t r_s, fp_t b, uint64_t seed) {
  const fp_t L{box_length_from_rs(r_s, N)};
  Config config{make_config(N, L, 3000U, 40000U, L / 10.0_fp, seed, 1000U)};
  config.jastrow_b = b;
...
TEST_CASE("Optimizer: produces valid b parameter", "[optimizer]") {
  ...
  REQUIRE(result.optimal_b > 0.0_fp);
...
TEST_CASE("Optimizer: optimized b lowers energy vs default b=1", "[optimizer]") {
  ...
  const fp_t E_OPTIMIZED{energy_at_b(N, R_S, result.optimal_b, 9999U)};
  const fp_t E_DEFAULT{energy_at_b(N, R_S, 1.0_fp, 9999U)};
...
TEST_CASE("Optimizer: works at r_s=10", "[optimizer]") {
  ...
  REQUIRE(std::isfinite(result.optimal_b));
  REQUIRE(result.optimal_b > 0.0_fp);
  REQUIRE(result.optimal_b < 20.0_fp);
}
